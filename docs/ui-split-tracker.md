...
| Item Lookup | `/item-lookup` | `lookup` | **done** | Option A (d4e66553) cherry-picked in first; legacy page, tablet and old desktop views removed (19 files, incl. the legacy `UpcScanner`); page moved up out of `lookup/dev/`, mobile page renamed `ItemLookupMobile`; own dev Redux state (`devItemLookupSlice`). Next: Option A gate per tree |
