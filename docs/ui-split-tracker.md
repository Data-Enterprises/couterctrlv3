...
| Item Actions | `/item-report` | `itemReport` | **done (dev-only)** | Coming Soon: `dev/` tree, empty `prod/` until greenlight; own dev Redux state (`devItemReportSlice`). The "See item actions" handoff (`hooks/useCriticalReport`) writes to the dev slice and its button is in the Sales / Sub Dept / Vendors **dev** trees only |
| Invoices | `/invoices` | `invoices` | **done (dev-only)** | Coming Soon: `dev/` tree, empty `prod/` until greenlight; its CLAUDE.md, sample AWG file and awg.html moved into `dev/` with the code; own dev Redux state (`devInvoicesSlice`) |
