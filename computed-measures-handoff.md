...
Two additions to `/sales/export` so a client's own query can become a file:
a **name** for a measure, and a **measure worked out from other measures**.
...
2. The tree walker — one function, `_render_expr(node, types, depth)`, returning
   the SQL fragment and appending any bound values. Everything else in §2 is ten
   lines around it.
3. `computed` in the request model and in `_resolve_export_aggregation`, next to
