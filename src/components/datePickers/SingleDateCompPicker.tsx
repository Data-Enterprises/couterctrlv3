...
const SingleDateCompPicker = ({ useSingleDate = true }: Props) => {
  ...
  const formatDisplay = () => {
    if (useSingleDate && subCompState.endDate) {
      return formatShortDate(subCompState.endDate);
    } else {
      return formatShortDate(new Date().toDateString());
    }
  };
  ...
  return (
    <Menu as="div" className="relative inline-block text-left w-full ">
      <div>
        <label className="block text-sm/6 font-medium ">{useSingleDate ? "Select Date" : "Select Dates"}</label>
        <MenuButton
          ref={menuRef}
...
export default SingleDateCompPicker;
