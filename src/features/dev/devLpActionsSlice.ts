/**
 * Dev copy of `features/lpActionsSlice.ts`, mounted at `state.dev.lpActions` and read only
 * by `pages/lpActions/dev`. Edit this one while changing dev lpActions; when dev is
...
const lpActionsSlice = createSlice({
  // Distinct from the prod slice's "lpActions": Redux matches actions on this
