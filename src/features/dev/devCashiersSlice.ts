/**
 * Dev copy of `features/cashiersSlice.ts`, mounted at `state.dev.cashier` and read only
 * by `pages/cashiers/dev`. Edit this one while changing dev cashiers; when dev is
...
const cashiersSlice = createSlice({
  // Distinct from the prod slice's "cashiers": Redux matches actions on this
