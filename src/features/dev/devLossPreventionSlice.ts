/**
 * Dev copy of `features/lossPreventionSlice.ts`, mounted at `state.dev.lossPrevention` and read only
 * by `pages/lossPrevention/dev`. Edit this one while changing dev lossPrevention; when dev is
...
export const lossPreventionSlice = createSlice({
  // Distinct from the prod slice's "lossPrevention": Redux matches actions on this
  // string alone, so sharing it would move prod and dev together.
  name: "devLossPrevention",
  initialState,
  reducers: {
