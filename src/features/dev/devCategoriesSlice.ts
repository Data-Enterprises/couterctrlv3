/**
 * Dev copy of `features/categoriesSlice.ts`, mounted at `state.dev.categories` and read only
 * by `pages/categories/dev`. Edit this one while changing dev categories; when dev is
...
const categoriesSlice = createSlice({
  // Distinct from the prod slice's "categories": Redux matches actions on this
