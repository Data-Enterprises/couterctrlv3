/**
 * Dev copy of `features/suggestedSlice.ts`, mounted at `state.dev.suggested` and read only
 * by `pages/suggested/dev`. Edit this one while changing dev suggested; when dev is
...
export const suggestedSlice = createSlice({
  // Distinct from the prod slice's "suggested": Redux matches actions on this
