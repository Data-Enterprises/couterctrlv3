/**
 * Dev copy of `features/itemReportSlice.ts`, mounted at `state.dev.itemReport` and read only
 * by `pages/itemReport/dev`. Edit this one while changing dev itemReport; when dev is
...
const itemReportSlice = createSlice({
  // Distinct from the prod slice's "itemReport": Redux matches actions on this
