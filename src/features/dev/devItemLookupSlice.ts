/**
 * Dev copy of `features/itemLookupSlice.ts`, mounted at `state.dev.item` and read only
 * by `pages/lookup/dev`. Edit this one while changing dev lookup; when dev is
...
const itemLookupSlice = createSlice({
  // Distinct from the prod slice's "itemLookup": Redux matches actions on this
