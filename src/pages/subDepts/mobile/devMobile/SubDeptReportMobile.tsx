  ...
  return (
    <div className="flex flex-col h-[calc(100dvh-3rem)] overflow-hidden bg-gray-50">
      ...
      >
        ...
        >
          <span
            className={`text-[10px] font-semibold leading-none ${selectedDay === null ? "text-custom-white" : "text-content"}`}
          >
            ALL
          </span>
          <span
            className={`text-[10px] ${selectedDay === null ? "text-custom-white/85" : "text-content/85"}`}
          >
            wk
          </span>
        </button>
      ...
      <div className="flex-shrink-0 flex items-center bg-custom-white border-b border-gray-100 px-3">
        <div className="py-2.5 text-[11px] font-semibold text-content border-b-2 border-[#1e2a4a]">
          Items
        </div>
        <div className="flex-1" />
        <div className="flex items-center gap-1.5 text-[10px] text-content/85">
          <span>Threshold</span>
          <input
            type="number"
        ...
        {visibleItems.map((item) => (
          ...
          >
            ...
            <div className="flex gap-2 mt-1.5 justify-end">
              {item.lwNet > 0 && (
                <span className={pillClass(item.lwPtsDelta)}>
                  LW {fmtPts(item.lwPtsDelta)}
                </span>
              )}
              <span
                className={pillClass(item.lyNet > 0 ? item.ptsDelta : null)}
              >
        ...
        <BottomSheet onClose={() => setSheetItem(null)}>
          <div className="flex items-start justify-between px-4 pb-3 border-b border-gray-100">
            <div className="min-w-0 flex-1">
              <div className="text-[13px] font-bold text-content truncate">
                {sheetItem.product_description}
              </div>
              <div className="text-[10px] text-content/85 mt-0.5">
                {sheetItem.product_code}
              </div>
            </div>
            <div
            ...
            >
              {sheetItem.tier === "critical" && (
                <ExclamationTriangleIcon
                  className="w-3 h-3"
                  style={{ color: BADGE_COLOR[sheetItem.tier] }}
                />
              )}
              ...
              <span
                ...
                style={{ color: BADGE_COLOR[sheetItem.tier] }}
              >
                {sheetItem.tier.charAt(0).toUpperCase() +
                  sheetItem.tier.slice(1)}
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 divide-x divide-gray-100 border-b border-gray-100">
            <div className="px-3 py-2.5">
              ...
              <div className="text-[13px] font-bold text-content mt-0.5">
                {formatCurrency2(sheetItem.tyNet)}
              </div>
              <div className="text-[10px] text-content/85 mt-0.5">
                {fmt1(sheetItem.tyMarginPct)}% margin
              </div>
              <div className="text-[10px] text-content/85">
                {sheetItem.tyQty.toFixed(0)} units
              </div>
            </div>
            <div className="px-3 py-2.5">
              <div className="text-[10px] font-semibold uppercase tracking-wide text-content/85">
              ...
              <div className="text-[13px] font-bold text-content mt-0.5">
                {formatCurrency2(sheetItem.lwNet)}
              </div>
              <div
                className={`text-[10px] font-semibold mt-0.5 ${sheetItem.lwPtsDelta >= 0 ? "text-emerald-600" : "text-red-600"}`}
              >
                {fmtPts(sheetItem.lwPtsDelta)}
              </div>
              <div className="text-[10px] text-content/85">
                {sheetItem.lwQty.toFixed(0)} units
              </div>
            </div>
            <div className="px-3 py-2.5">
              <div className="text-[10px] font-semibold uppercase tracking-wide text-content/85">
              ...
              <div className="text-[13px] font-bold text-content mt-0.5">
                {formatCurrency2(sheetItem.lyNet)}
              </div>
              <div
                className={`text-[10px] font-semibold mt-0.5 ${sheetItem.ptsDelta >= 0 ? "text-emerald-600" : "text-red-600"}`}
              >
                {fmtPts(sheetItem.ptsDelta)}
              </div>
              <div className="text-[10px] text-content/85">
                {sheetItem.lyQty.toFixed(0)} units
              </div>
            </div>
          </div>
