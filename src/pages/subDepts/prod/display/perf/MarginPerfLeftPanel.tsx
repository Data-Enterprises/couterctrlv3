...
const MarginPerfLeftPanel = ({ onSearchOpen, onStoreNumberChange }: Props) => {
  ...
  const rawGradingThreshold = useAppSelector((s) => s.prod.subMargin.gradingThreshold);
  ...
  const loadingGrades = useAppSelector((s) => s.prod.subMargin.loadingGrades);
  const storeSalesTotals = useAppSelector((s) => s.prod.subMargin.storeSalesTotals);

  const availableStoreNumbers = useAppSelector(
    (s) => s.prod.subMargin.availableStoreNumbers,
  );
