...
const DeltaBadge = ({
  ...
  if (!lyVal && unit === "pct")
    return (
      <span className="text-[9px] font-semibold mt-0.5 text-content/25">—</span>
    );
  const delta =
    unit === "pts" ? tyVal - lyVal : ((tyVal - lyVal) / Math.abs(lyVal)) * 100;
  const isUp = delta >= 0;
  return (
    <span
...
const SmDevDaySidebar = () => {
  ...
  const lyByDate = useMemo(() => {
    ...
    for (const [d, v] of Object.entries(map)) {
      result[d] = {
        sales: v.sales,
        marginPct: v.sales > 0 ? ((v.sales - v.cogs) / v.sales) * 100 : 0,
      };
    }
    return result;
  }, [lyMargins]);
  ...
  const handleDayClick = (isoDate: string) => {
    ...
    dispatch(
      actions.setSelectedWeekDay(
        ctx.selectedWeekDay === formatted ? "" : formatted,
      ),
    );
  };
  ...
  return (
    <div className="flex border-b border-gray-100">
      ...
      {dates.map((isoDate) => {
        ...
        const twDay = agg(
          ctx.margins.filter((m) => m.sale_date.split("T")[0] === isoDate),
        );
        ...
        return (
          <div
            ...
            className={`flex flex-col items-center justify-center px-1 py-2 border-r border-gray-100 last:border-r-0 transition-colors cursor-pointer select-none flex-1 ${
              isSelected ? "bg-gray-100" : "bg-custom-white hover:bg-gray-50"
            }`}
