...
const SmDevRightPanel = () => {
  ...
  const weekHeader = () => {
    ...
    const wkMargins = [ctx.weekOneMargins, ctx.weekTwoMargins, ctx.weekThreeMargins, ctx.weekFourMargins][ctx.selectedWeek - 1];
  ...
  const kpiValues = weekKpis
    ? [weekKpis.sales, weekKpis.cogs, weekKpis.margin, weekKpis.qty, weekKpis.items]
    : [];
    ...
    return (
      <div className="flex-1 min-w-0 shadow-lg">
        <div className="bg-custom-white rounded-xl shadow-sm overflow-hidden flex flex-col h-full items-center justify-center gap-2">
          ...
          <p className="text-[11px] text-content/25">Choose one from the list on the left</p>
        </div>
      </div>
    );
  }
  ...
  if (ctx.selectedWeek === 0) {
    return (
      <div className="flex-1 min-w-0 shadow-lg">
        <div className="bg-custom-white rounded-xl shadow-sm overflow-hidden flex flex-col h-full items-center justify-center gap-2">
          <p className="text-[13px] font-medium text-content/40">Select a week</p>
          <p className="text-[11px] text-content/25">
            {subDeptName ? `Viewing ${subDeptName}` : "Choose a week from the left panel"}
          </p>
        </div>
      </div>
    );
      ...
      <div className="bg-custom-white rounded-xl shadow-sm overflow-hidden flex flex-col h-full">
        ...
        {ctx.selectedWeek < 5 ? (
          <>
            {/* ── Summary KPI strip ── */}
            {weekKpis && (() => {
              const twRaws = [weekKpis.rawSales, weekKpis.rawCogs, weekKpis.rawMargin, weekKpis.rawQty, weekKpis.rawItems];
              const lyRaws = lyKpis ? [lyKpis.rawSales, lyKpis.rawCogs, lyKpis.rawMargin, lyKpis.rawQty, lyKpis.rawItems] : [];
              return (
                <div className="grid grid-cols-5 divide-x divide-gray-100 border-b border-gray-100 flex-shrink-0">
                  {KPI_LABELS.map((label, i) => {
                    ...
                    const ly = lyRaws[i];
                    const isMargin = i === 2;
                    const delta = ly ? (isMargin ? tw - ly : ((tw - ly) / Math.abs(ly)) * 100) : null;
                    const isUp = delta !== null && delta > 0;
                    return (
                      <div key={label} className="px-3.5 py-[11px] bg-white">
                        <div className="text-[8px] font-bold uppercase tracking-[.07em] text-content/40 mb-1">{label}</div>
                        <div className="text-[15px] font-bold text-[#1e2a4a] leading-none">{kpiValues[i]}</div>
                        <div className="text-[9px] text-content/40 mt-1">
                          LY <span className="text-content/60 font-medium">{lyKpiValues.length ? lyKpiValues[i] : "—"}</span>
                        </div>
                        {delta !== null && (
                          <span
                          ...
                          >
                            {isUp ? "▲" : "▼"} {Math.abs(delta).toFixed(1)}{isMargin ? " pts" : "%"} vs LY
                          </span>
                        )}
                      </div>
                    );
