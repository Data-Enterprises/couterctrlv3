...
export const LOOKUP_INFO: { title: string; purpose: string; glossary: InfoGlossaryEntry[] } = {
  ...
  glossary: [
    { term: "Margin %", desc: "Amount less cost of goods, calculated fresh for this store over the last 14 days. Not carried over from wherever the UPC was copied from, so the same item can show different margins at different stores." },
    ...
    { term: "Units vs. scans", desc: "On a scale item one scan can be several pounds — bananas ring about 2 lb at a time. Cost and margin count what was actually sold, not how many times the item was rung up. The daily breakdown shows both: Qty is the scans, Weight is the pounds." },
    { term: "Sale types", desc: "Every register line is a Sale, Backup (a transaction that was restored), Cancelled or Voided. Amount, margin, days sold and the daily breakdown count Sale lines only; the others are listed under By sale type, each as a share of sales." },
    { term: "Sale type timeline", desc: "Select a sale type to see its days in the daily breakdown. Cost and margin are left off for Backup, Cancelled and Voided, since they aren't sales. Select it again, or Back to sales, to return." },
    { term: "Avg sold at", desc: "Amount divided by what actually sold, so it stays comparable to the shelf price. Sits below it when the item spent part of the window on promotion." },
