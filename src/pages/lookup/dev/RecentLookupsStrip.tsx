...
const marginColor = (marginPct: number | null) => {
  if (marginPct === null) return "text-content/60";
  return marginPct < 0 ? "text-red-700" : "text-emerald-700";
};

const RecentLookupsStrip = ({ onSelect }: RecentLookupsStripProps) => {
  const { recentLookups } = useAppSelector((s) => s.item);
  if (!recentLookups.length) return null;

  return (
    <div>
      <div className="text-[9px] font-semibold text-content/45 uppercase tracking-wide mb-2">
        Recent lookups
      </div>
