...
const PopupHourlyView = ({
  ...
  const { hourlySales, hourlySalesLastWeek, hourlySalesLastYear } =
    useAppSelector((s) => s.dev.sales);
  const { rawHourly, rawLWHourly, rawLYHourly } = useAppSelector(
    (s) => s.dev.salesLedger,
  );
  const rawThreshold = useAppSelector((s) => s.dev.salesLedger.hourlyThreshold);
  const selectedHour = useAppSelector((s) => s.dev.salesLedger.selectedHour);
  const gradingMetric = useAppSelector((s) => s.dev.salesLedger.gradingMetric);
