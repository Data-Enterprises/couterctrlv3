...
const PopupSubDeptList = ({
  ...
  const rawThreshold = useAppSelector(
    (state) => state.dev.salesLedger.subDeptThreshold,
  );
  const rawItemThreshold = useAppSelector(
    (state) => state.dev.salesLedger.itemThreshold,
  );
  const gradingMetric = useAppSelector(
    (state) => state.dev.salesLedger.gradingMetric,
  );
  const isQty = gradingMetric === "qty";
  const selectedId = useAppSelector(
    (state) => state.dev.salesLedger.selectedSubDeptId,
  );
  const items = useAppSelector(
    (state) => state.dev.salesLedger.selectedSubDeptItems,
  );
  const inactiveItems = useAppSelector(
    (state) => state.dev.salesLedger.inactiveSubDeptItems,
  );
  const lastFetchedItemsKey = useAppSelector(
    (state) => state.dev.salesLedger.lastFetchedItemsKey,
  );
