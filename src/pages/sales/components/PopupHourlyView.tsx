...
const getCta = (row: HourRow, threshold: number): { text: string; severity: Severity } => {
  ...
  const refBasket = row.hasLY
    ? (row.lyTrans > 0 ? row.ly / row.lyTrans : 0)
    : (row.lwTrans > 0 ? row.lw / row.lwTrans : 0);

  if (sev === "critical") {
    const transDrop = refTrans > 0 ? ((row.trans - refTrans) / refTrans) * 100 : null;
    const basketDiff = refBasket > 0 ? ((avgBasket - refBasket) / refBasket) * 100 : null;
    const isTrafficLoss = transDrop !== null && transDrop < -5;
    const isSpendDrop = basketDiff !== null && basketDiff < -3;
    if (isTrafficLoss && !isSpendDrop)
      return { severity: "critical", text: `Down ${pctStr} vs ${primaryPeriod} — traffic loss is the driver. Transactions down ${Math.abs(transDrop!).toFixed(1)}% while basket is holding. Check staffing and flow.` };
    if (isSpendDrop && !isTrafficLoss)
      return { severity: "critical", text: `Down ${pctStr} vs ${primaryPeriod} — spend compression is the driver. Traffic held but avg basket dropped. Look at mix shift or promoted item performance.` };
    return { severity: "critical", text: `Down ${pctStr} vs ${primaryPeriod} — exceeds the 9% threshold. Both traffic and spend show pressure. Investigate staffing, promotions, and item availability.` };
  }
  if (sev === "watch") {
    const secondaryNote = row.hasLY && row.hasLW
      ...
      : "";
    return { severity: "watch", text: `Down ${pctStr} vs ${primaryPeriod} — within the watch band.${secondaryNote}` };
  }
  const secondaryHealthNote = row.hasLY && row.hasLW
    ? row.vsLWPct < 0 ? ` LW is softer — watch for a developing trend.` : ` LW also positive.`
    : "";
  return { severity: "healthy", text: `At or above ${primaryPeriod}.${secondaryHealthNote} Traffic and spend contributing positively.` };
};
...
const PopupHourlyView = ({ twDateLabel, lwDateLabel, lyDateLabel }: PopupHourlyViewProps) => {
  ...
  const hours = useMemo((): HourRow[] => {
    ...
    return allHours
      ...
      .sort((a, b) => {
        ...
        const rankDiff = rank[hourSeverity(a, threshold)] - rank[hourSeverity(b, threshold)];
  ...
  const critCount = hours.filter((h) => hourSeverity(h, threshold) === "critical").length;
  const watchCount = hours.filter((h) => hourSeverity(h, threshold) === "watch").length;
  const healthyCount = hours.filter((h) => hourSeverity(h, threshold) === "healthy").length;

  const visible = hours.filter((h) => {
    if (sevFilter === "critical") return hourSeverity(h, threshold) === "critical";
    if (sevFilter === "watch") return hourSeverity(h, threshold) === "watch";
    if (sevFilter === "healthy") return hourSeverity(h, threshold) === "healthy";
    return true;
  });
    ...
    <div className="flex h-full">
      ...
      <div className="flex flex-col border-r border-gray-100" style={{ width: "40%" }}>
        ...
        <div className="flex flex-wrap gap-1 p-2 border-b border-gray-100 bg-gray-100">
          <button onClick={() => setSevFilter("all")} className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium transition-colors border ${chipClass(sevFilter === "all")}`}>All ({hours.length})</button>
          ...
          {visible.map((r) => {
            ...
            return (
              <button
                ...
                style={isSel ? { boxShadow: "inset 4px 0 0 rgba(30, 42, 74, 0.5)" } : undefined}
              >
                <div className="flex items-center gap-2">
                  <SeverityBadge severity={sev} />
                  <div className="flex-1 min-w-0">
                    <div className="text-[12px] font-medium text-content">{ampm(r.hour)}</div>
                    <div className={`text-[9px] ${isSel ? "text-content/50" : "text-content/45"}`}>{formatHourRange(r.hour)}</div>
                  </div>
                  <div className="flex items-baseline gap-1.5 flex-shrink-0">
                    <span className="text-[12px] font-semibold text-content">{formatCurrency2(r.tw)}</span>
                    <span className="text-[10px] text-content/60">{r.qty.toLocaleString()} u</span>
                  </div>
                </div>
        ...
        {selected ? (
          <>
            ...
            <div className="px-4 py-2 border-b border-gray-100 bg-gray-100 flex items-baseline gap-2">
              <span className="text-[13px] font-semibold text-content">{ampm(selected.hour)}</span>
              <span className="text-[10px] text-content/45 italic">{formatHourRange(selected.hour)} · {twDateLabel}</span>
            </div>
            ...
            <div className="flex-1 overflow-y-auto thin-scrollbar">
              ...
              <div className="border-b border-gray-100">
                <div className="px-4 py-1.5 bg-gray-100 text-[10px] font-medium uppercase tracking-wide text-content/55">Transactions</div>
                <div className="grid grid-cols-3 divide-x divide-gray-100">
                  <div className="px-4 py-2.5">
                    <div className="text-[13px] font-semibold text-content">{selected.trans.toLocaleString()}</div>
                  </div>
                  <div className="px-4 py-2.5">
                    <div className="flex items-baseline gap-1.5">
                      <span className="text-[13px] font-semibold text-content">{selected.lwTrans > 0 ? selected.lwTrans.toLocaleString() : "—"}</span>
                      {selected.lwTrans > 0 && (
                        <span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded ${pillClass(((selected.trans - selected.lwTrans) / selected.lwTrans) * 100, threshold)}`}>
                          {formatPct(((selected.trans - selected.lwTrans) / selected.lwTrans) * 100)}
                        </span>
                      )}
                    </div>
                  </div>
                  <div className="px-4 py-2.5">
                    <div className="flex items-baseline gap-1.5">
                      <span className="text-[13px] font-semibold text-content">{selected.lyTrans > 0 ? selected.lyTrans.toLocaleString() : "—"}</span>
                      {selected.lyTrans > 0 && (
                        <span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded ${pillClass(((selected.trans - selected.lyTrans) / selected.lyTrans) * 100, threshold)}`}>
                          {formatPct(((selected.trans - selected.lyTrans) / selected.lyTrans) * 100)}
                        </span>
                      )}
                    </div>
                  </div>
              ...
              {/* Avg basket — 3-col */}
              <div className="border-b border-gray-100">
                <div className="px-4 py-1.5 bg-gray-100 text-[10px] font-medium uppercase tracking-wide text-content/55">Avg basket</div>
                <div className="grid grid-cols-3 divide-x divide-gray-100">
                  <div className="px-4 py-2.5">
                    <div className="text-[13px] font-semibold text-content">{formatCurrency2(avgBasket)}</div>
                  </div>
                  <div className="px-4 py-2.5">
                    <div className="flex items-baseline gap-1.5">
                      <span className="text-[13px] font-semibold text-content">{lwAvgBasket > 0 ? formatCurrency2(lwAvgBasket) : "—"}</span>
                      {lwAvgBasket > 0 && (
                        <span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded ${pillClass(((avgBasket - lwAvgBasket) / lwAvgBasket) * 100, threshold)}`}>
                  ...
                  <div className="px-4 py-2.5">
                    <div className="flex items-baseline gap-1.5">
                      <span className="text-[13px] font-semibold text-content">{lyAvgBasket > 0 ? formatCurrency2(lyAvgBasket) : "—"}</span>
                      {lyAvgBasket > 0 && (
                        <span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded ${pillClass(((avgBasket - lyAvgBasket) / lyAvgBasket) * 100, threshold)}`}>
                          {formatPct(((avgBasket - lyAvgBasket) / lyAvgBasket) * 100)}
                        </span>
                      )}
                    </div>
                  </div>
