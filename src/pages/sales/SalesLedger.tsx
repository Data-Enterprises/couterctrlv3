...
const SalesLedger = () => {
  ...
  const filtered = ledgerRows.filter((r) => {
    if (filter === "attention") return r.vsLYPct < -2;
    if (filter === "above") return r.vsLYPct > 2;
    return true;
  });
  ...
  const attentionCount = ledgerRows.filter((r) => r.vsLYPct < -2).length;
  ...
  return (
    <div className="w-full h-[calc(100vh-3rem)] flex flex-col p-4 select-none overflow-hidden">
      {loading ? (
        <div className="relative flex-1">
          <LoadingIndicator message="Loading store ledger" />
        </div>
      ) : ledgerRows.length === 0 ? (
        <div className="flex-1 flex items-center justify-center text-content/40 text-sm">
          No data found for this period.
        </div>
      ) : (
        <div className="w-1/2 mx-auto flex flex-col flex-1 min-h-0">
          <LedgerHeroBar
            weekLabel={weekLabel}
            twTotal={heroTWTotal}
            vsLYPct={heroVsLYPct}
          ...
          <div className="bg-custom-white rounded-xl shadow-sm overflow-hidden flex flex-col flex-1 min-h-0">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 text-content/40 text-[11px] uppercase tracking-wide">
                  ...
                  <th className="text-center px-4 py-2 w-12">Status</th>
                </tr>
              </thead>
            </table>
      ...
      {/* Popup placeholder — wired next */}
      {selection !== null && (
        <div className="fixed bottom-4 right-4 text-xs text-content/40 bg-custom-white px-3 py-1.5 rounded-lg shadow">
          {selection.storeName} · {selection.mode} · {selection.start}{selection.mode === "weekly" ? ` – ${selection.end}` : ""}
        </div>
      )}
    </div>
  );
