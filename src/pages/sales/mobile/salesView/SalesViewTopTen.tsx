      ...
      <div>
        {ctx.salesViewTopTen.map((item, i) => (
          ...
          >
            ...
            <div>
              <div className="text-[10px] text-content/85">Sales</div>
              <div className="font-medium">
                {formatCurrency2(item.total_sales)}
              </div>
            </div>
            <div>
              <div className="text-[10px] text-content/85">Qty</div>
              <div className="font-medium">{formatBigNumber(item.qty, 0)}</div>
            </div>

            {/* Column 2 */}
            <div>
              <div className="text-[10px] text-content/85">COGS</div>
              <div className="font-medium">{formatCurrency2(item.cost)}</div>
            </div>
            <div>
              <div className="text-[10px] text-content/85">Profit</div>
              <div className="font-medium">
            ...
            <div>
              <div className="text-[10px] text-content/85">GPM</div>
              <div className="font-medium">
                {gpm(item.total_sales, item.cost)}
              </div>
            </div>
            <div>
              <div className="text-[10px] text-content/85">RPU</div>
              <div className="font-medium">
                {formatCurrency2(rpu(item.total_sales, item.qty))}
              </div>
            </div>
            <div>
              <div className="text-[10px] text-content/85">PPU</div>
              <div className="font-medium">
                {formatCurrency2(ppu(item.total_sales, item.cost, item.qty))}
              </div>
            </div>
            <div>
              <div className="text-[10px] text-content/85">CPU</div>
              <div className="font-medium">
