...
const PopupHourlyView = ({
  ...
  const { hourlySales, hourlySalesLastWeek, hourlySalesLastYear } =
    useAppSelector((s) => s.prod.sales);
  const { rawHourly, rawLWHourly, rawLYHourly } = useAppSelector(
    (s) => s.prod.salesLedger,
  );
  const rawThreshold = useAppSelector((s) => s.prod.salesLedger.hourlyThreshold);
  const selectedHour = useAppSelector((s) => s.prod.salesLedger.selectedHour);
  const gradingMetric = useAppSelector((s) => s.prod.salesLedger.gradingMetric);
