...
const CouponsExportModal = ({
  onClose,
  title,
  subtitle,
  rows,
}: CouponsExportModalProps) => {
  ...
  const uniqueStores = useMemo(() => {
    ...
    rows.forEach((r) => {
      if (!map.has(r.store_number))
        map.set(r.store_number, {
          name: r.store_name ?? r.store_number,
  ...
  const filteredRows = useMemo(
    () =>
      isMultiStore
        ? rows.filter((r) => selectedStores.has(r.store_number))
        : rows,
  ...
  const flatRows = useMemo<AggRow[]>(
    () => filteredRows.map((r) => ({ ...r }) as unknown as AggRow),
    [filteredRows],
  );
  ...
  const StoreFilter = () => (
    <div>
      <div className="flex items-center justify-between mb-2">
        <p className="text-[10px] font-semibold uppercase tracking-wide text-content/45">
          Stores
        </p>
        <div className="flex gap-1.5">
          <button
            onClick={() =>
              setSelectedStores(new Set(uniqueStores.map((s) => s.number)))
            }
            disabled={allSelected}
            className="text-[9px] text-[#1e2a4a]/50 hover:text-[#1e2a4a] disabled:opacity-30 transition-colors"
          >
            All
          </button>
      ...
      <div className="space-y-1.5 max-h-36 overflow-y-auto thin-scrollbar pr-1">
        {uniqueStores.map((s) => (
          ...
          >
            ...
            <span className="text-[10px] text-content/40 flex-shrink-0">
              #{s.number}
            </span>
          </label>
        ))}
  ...
  return (
    <ResizableModalShell
      onClose={onClose}
      storageKey="export-modal:coupons:v2"
      defaultWidth={1140}
      defaultHeight={960}
    >
        {/* Header */}
          ...
          <div className="flex items-center gap-0.5 bg-custom-white/10 rounded-md p-0.5">
            {(["presets", "custom"] as ModalMode[]).map((m) => (
              <button
                key={m}
                onClick={() => setMode(m)}
                className={`px-2.5 py-1 rounded text-[11px] font-medium transition-colors ${mode === m ? "bg-custom-white text-[#1e2a4a]" : "text-custom-white/70 hover:text-custom-white"}`}
              >
        ...
        {mode === "presets" && (
          <>
            <div className="px-4 pt-4 pb-2 space-y-4">
              ...
              <div>
                {!isMultiStore && (
                  <p className="text-[11px] text-content/50 uppercase tracking-wide font-medium mb-3">
                ...
                <label className="flex items-start gap-3 cursor-pointer group">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-content/50 mt-0.5">
                      ...
                      {isMultiStore && selectedStores.size < uniqueStores.length
                        ? ` · ${selectedStores.size} of ${uniqueStores.length} stores`
                        : ""}
                      {" · "}
                      {title}
                    </p>
            ...
            <div className="grid grid-cols-[220px_1fr] divide-x divide-gray-100 min-h-[360px] max-h-[calc(100vh-220px)]">
              ...
              <div className="overflow-y-auto no-scrollbar p-4 space-y-5">
                ...
                <p className="text-[10px] text-content/35 italic leading-relaxed">
                  {filteredRows.length} rows
                  {isMultiStore && selectedStores.size < uniqueStores.length
                    ? ` · ${selectedStores.size}/${uniqueStores.length} stores`
                    : ""}
                </p>
              </div>
                      ...
                      <tbody>
                        ...
                        {aggRows.length > PREVIEW_ROWS && (
                          <tr>
                            ...
                            >
                              + {aggRows.length - PREVIEW_ROWS} more rows…
                            </td>
                          </tr>
                        )}
                      </tbody>
