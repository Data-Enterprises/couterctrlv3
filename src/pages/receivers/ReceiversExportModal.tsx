...
import type { ReceiverDetailsItem, ReceiverDetailsTotals } from "../../interfaces";
...
const buildDetailsCsv = (details: ReceiverDetailsItem[], label: string) => {
  const headers = [
    "Line #", "UPC", "Description",
    "Cases", "Units", "U Cost", "Ext Cost",
    "Retail", "Ext Retail", "GM", "Free", "Return",
  ];
  const rows = details.map((d) => [
    d.line_number, d.product_code, d.product_description,
    d.cases, d.units, fmtNum(d.ucost), fmtNum(d.ext_cost),
    fmtNum(d.retail), fmtNum(d.ext_retail), fmtNum(d.gm),
    d.free, d.return,
  ]);
...
const ReceiversExportModal = ({
  ...
  const handlePresetDownload = () => {
    ...
    downloadCsv(buildDetailsCsv(details, label), `${safeName}_${invoiceId}.csv`);
    onClose();
  };
  ...
  const handleCustomDownload = () => {
    ...
    const safeName = vendorName.replace(/[^a-z0-9]/gi, "_");
    downloadCsv(rowsToCsv(headers, rows), `${safeName}_${invoiceId}_custom.csv`);
    onClose();
  };
        ...
        <div className="flex items-center justify-between px-4 py-3 bg-[#1e2a4a]">
          <div className="flex items-center gap-3">
            <div>
              ...
              <p className="text-white/55 text-[10px] mt-0.5">{vendorName} — Ref #{referenceNumber}</p>
            </div>
              ...
              <label className="flex items-start gap-3 cursor-pointer group">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-content/50 mt-0.5">
                    {details.length} line items for {vendorName} — Invoice {invoiceId}
                    {totals && ` · Cases ${totals.cases} · Units ${totals.units}`}
                  </p>
                </div>
              </label>
            </div>
            ...
            <div className="grid grid-cols-[200px_1fr] divide-x divide-gray-100 min-h-[360px] max-h-[calc(100vh-220px)]">
              ...
              <div className="overflow-y-auto no-scrollbar p-4 space-y-5">
                ...
                <p className="text-[10px] text-content/35 italic leading-relaxed">
                  Working on {details.length} line items for {vendorName} Invoice {invoiceId}.
                </p>
              </div>
