...
const CashiersMobileList = ({
  ...
  const counts: Record<SevFilter, number> = {
    ...
    critical: ctx.cashierCards.filter((c) => toSev(c.risk_tier) === "critical")
      .length,
    watch: ctx.cashierCards.filter((c) => toSev(c.risk_tier) === "watch")
      .length,
    healthy: ctx.cashierCards.filter((c) => toSev(c.risk_tier) === "healthy")
      .length,
  };
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto thin-scrollbar">
        ...
        {!ctx.loadingCashiers &&
          visible.length === 0 &&
          ctx.cashierCards.length > 0 && (
            <div className="flex items-center justify-center py-16 text-[12px] text-content/85">
              No cashiers match filter.
            </div>
          )}
        {!ctx.loadingCashiers &&
          visible.map((cashier) => {
              ...
              >
                ...
                <div className="flex items-center gap-1.5">
                  {[
                    {
                      label: "Total",
                      value: formatCurrency2(cashier.total_sales),
                    },
                    { label: "Net", value: formatCurrency2(cashier.net_sales) },
                    {
                      label: "Qty",
                      value: formatBigNumber(cashier.total_qty, 0),
                    },
                  ].map(({ label, value }) => (
                    <div
                      key={label}
