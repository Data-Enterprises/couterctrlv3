  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      >
        ...
        <button
          onClick={onBack}
          className="text-[10px] px-2.5 py-1 rounded text-custom-white/70 hover:text-custom-white transition-colors"
      ...
      {ctx.fetchingTransactions ? (
        ...
        <div className="flex flex-1 min-h-0">
          ...
          >
            ...
            <div className="flex-1 min-h-0 overflow-y-auto thin-scrollbar">
              {overviews.map((ov) => {
                const isSelected =
                  ov.transaction_id === selectedOverview.transaction_id;
                return (
                  <button
                  ...
                  >
                    <div
                      ...
                      style={{
                        color: isSelected ? "#1e2a4a" : "rgba(30,42,74,0.55)",
                      }}
                    >
                      {ov.transaction_id}
                    </div>
                    <div className="text-[8.5px] text-content/55 truncate mt-0.5">
                      {ov.cashier_name}
                    </div>
                  </button>
                );
        ...
        <>
          ...
          >
            {["Trans ID", "Date", "Cashier", "Store", "Qty", "Total"].map(
              (h, i) => (
                <div
                  key={h}
          ...
          <div className="flex-1 min-h-0 overflow-y-auto thin-scrollbar">
            {overviews.map((ov, i) => (
              ...
              >
                <div
                  className="text-[10px] font-semibold underline truncate"
                  style={{ color: "#1e2a4a", textUnderlineOffset: 2 }}
                >
                  #{ov.transaction_id}
                </div>
                <div className="text-[10px] text-content/70">
                  {fmtDate(ov.sale_date)}
                </div>
                <div className="text-[10px] text-content/70 truncate">
                  {ov.cashier_name}
                </div>
                <div className="text-[10px] text-content/70">
                  {ov.store_number}
                </div>
                <div className="text-[10px] text-content/70 text-right">
                  {formatBigNumber(ov.qty, 0)}
                </div>
                <div className="text-[10px] font-medium text-content text-right">
