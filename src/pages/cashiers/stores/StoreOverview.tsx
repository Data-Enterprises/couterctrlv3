  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex-shrink-0 flex items-center justify-between px-4 py-2.5" style={{ background: "#1e2a4a" }}>
        <div>
          <div className="text-[13px] font-medium text-white">{store.store_name}</div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <button
            onClick={getCCards}
            className="text-[10px] px-2.5 py-1 rounded text-white transition-colors"
      ...
      <div className="flex-shrink-0 grid grid-cols-4 divide-x divide-gray-100 border-b border-gray-100">
        {[
          ...
          { label: "Transactions",value: formatBigNumber(store.total_transactions, 0) },
        ].map(({ label, value }) => (
          <div key={label} className="px-4 py-2.5">
            <div className="text-[9px] uppercase tracking-wide font-medium text-content/50">{label}</div>
            <div className="text-[16px] font-medium text-content mt-0.5">{value}</div>
          </div>
        ))}
      </div>
      ...
      <div className="flex-shrink-0 flex items-center justify-end gap-3 px-4 py-2.5 border-t border-gray-100">
        <span className="text-[9px] text-content/45 uppercase tracking-wide">Risk</span>
        <span className="text-[9px] font-medium px-2 py-0.5 rounded" style={{ background: rc.bg, color: rc.color }}>
          {store.risk_tier}
        </span>
        <span className="text-[9px] text-content/45 uppercase tracking-wide">Exception tier</span>
        <span className="text-[9px] font-medium px-2 py-0.5 rounded" style={{ background: rc.bg, color: rc.color }}>
