...
const CashiersView = ({ selectedCashier, onCashierSelect, onBackToStores }: Props) => {
  ...
  useEffect(() => {
    if (ctx.applyFilters) {
      ...
      const handleThreshold = (card: CashierCard, exc: ExceptionType, filter: NumberFilter, metric: "sales" | "qty") => {
        const formattedKey = `${exc.toLowerCase().replace(/\s/g, "_")}_${metric}` as keyof CashierCard;
      ...
      const filtered = ctx.cashierCards.filter((card) => {
        const matchName = cashName.length ? card.store_name.toLowerCase().includes(cashName.toLowerCase()) : true;
        const matchRisk = riskLvl.length ? card.risk_tier.toLowerCase() === riskLvl.toLowerCase() : true;
        const matchExc = excTier.length ? card.exception_tier.toLowerCase().includes(excTier.toLowerCase()) : true;
        return (
          matchName && matchRisk && matchExc &&
          (!salesTypes.includes("Adjustment") || !salesOp || handleThreshold(card, "Adjustment", totalSales, "sales")) &&
          (!qtyTypes.includes("Adjustment") || !qtyOp || handleThreshold(card, "Adjustment", totalQty, "qty")) &&
          (!salesTypes.includes("Backup") || !salesOp || handleThreshold(card, "Backup", totalSales, "sales")) &&
          (!qtyTypes.includes("Backup") || !qtyOp || handleThreshold(card, "Backup", totalQty, "qty")) &&
          (!salesTypes.includes("Cancelled") || !salesOp || handleThreshold(card, "Cancelled", totalSales, "sales")) &&
          (!qtyTypes.includes("Cancelled") || !qtyOp || handleThreshold(card, "Cancelled", totalQty, "qty")) &&
          (!salesTypes.includes("Hand Key") || !salesOp || handleThreshold(card, "Hand Key", totalSales, "sales")) &&
          (!qtyTypes.includes("Hand Key") || !qtyOp || handleThreshold(card, "Hand Key", totalQty, "qty")) &&
          (!salesTypes.includes("Modified") || !salesOp || handleThreshold(card, "Modified", totalSales, "sales")) &&
          (!qtyTypes.includes("Modified") || !qtyOp || handleThreshold(card, "Modified", totalQty, "qty")) &&
          (!salesTypes.includes("No Sale") || !salesOp || handleThreshold(card, "No Sale", totalSales, "sales")) &&
          (!qtyTypes.includes("No Sale") || !qtyOp || handleThreshold(card, "No Sale", totalQty, "qty")) &&
          (!salesTypes.includes("Refunded") || !salesOp || handleThreshold(card, "Refunded", totalSales, "sales")) &&
          (!qtyTypes.includes("Refunded") || !qtyOp || handleThreshold(card, "Refunded", totalQty, "qty")) &&
          (!salesTypes.includes("Voided") || !salesOp || handleThreshold(card, "Voided", totalSales, "sales")) &&
  ...
  const storeName = ctx.storeCards.find((s) => s.storeid === ctx.selectedStoreCard)?.store_name ?? "";
  ...
  const CashierRow = ({ cashier }: { cashier: CashierCard }) => {
    ...
    return (
      ...
      >
        <div className="text-[11px] font-medium text-content truncate w-full">{cashier.cashier_name}</div>
        <div className="flex items-baseline gap-1 rounded px-1 py-0.5 mt-0.5 self-start" style={{ background: "rgba(30,42,74,0.06)", boxShadow: "inset 0 1px 2px rgba(30,42,74,0.08)" }}>
          ...
          <span className="text-[9px] font-semibold text-content">{cashier.total_transactions}</span>
        </div>
      </button>
    );
  };
  ...
  return (
    <>
      ...
      <div className="flex-shrink-0 px-4 py-2.5" style={{ background: "#1e2a4a" }}>
        <div className="text-[13px] font-medium text-white truncate">{storeName}</div>
          ...
          <div className="flex-1 min-h-0 grid grid-cols-3 divide-x divide-gray-100 overflow-hidden">
            <div className="overflow-y-auto thin-scrollbar">
              {critical.length === 0
                ...
                : critical.map((c) => <CashierRow key={c.cashier_number} cashier={c} />)}
            </div>
            <div className="overflow-y-auto thin-scrollbar">
              {watch.length === 0
                ? <div className="flex items-center justify-center h-12 text-[10px] text-content/30">None</div>
                : watch.map((c) => <CashierRow key={c.cashier_number} cashier={c} />)}
            </div>
            <div className="overflow-y-auto thin-scrollbar">
              {healthy.length === 0
                ? <div className="flex items-center justify-center h-12 text-[10px] text-content/30">None</div>
                : healthy.map((c) => <CashierRow key={c.cashier_number} cashier={c} />)}
            </div>
          </div>
        </>
      )}
