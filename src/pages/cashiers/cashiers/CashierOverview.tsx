  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex-shrink-0 flex items-center justify-between px-4 py-2.5" style={{ background: "#1e2a4a" }}>
        <div>
          <div className="text-[13px] font-medium text-white">{cashier.cashier_name}</div>
          <div className="text-[10px] mt-0.5" style={{ color: "rgba(255,255,255,0.5)" }}>
            #{cashier.cashier_number} · {cashier.store_name}
          </div>
        </div>
        <span className="text-[8px] font-medium px-1.5 py-0.5 rounded flex-shrink-0" style={{ background: rc.bg, color: rc.color }}>
      ...
      <div className="flex-shrink-0 grid grid-cols-4 divide-x divide-gray-100 border-b border-gray-100">
        {[
          ...
          { label: "Transactions", value: formatBigNumber(cashier.total_transactions, 0) },
        ].map(({ label, value }) => (
