...
const StepReview = ({ onSubmit, onEdit }: StepReviewProps) => {
  ...
  const companyNames = ctx.companies
    .filter((c) =>
      ctx.selectedNewUserStores.some((s) => s.company === c.company),
    )
    .map((c) => c.name)
    .join(", ");
  const bgNames = ctx.selectedBaseGroups
    .filter((bg) =>
      ctx.selectedNewUserStores.some((s) => s.base_group === bg.id),
    )
    .map((bg) => bg.name)
    .join(", ");
      ...
      <div className="flex flex-col gap-2.5 mb-4">
        <div className="flex justify-between border-b border-gray-100 pb-2">
          ...
          <span className="text-[11px] text-content/70 text-right">
            {ctx.userInfo.username} · {ctx.userInfo.first_name}{" "}
            {ctx.userInfo.last_name} · {roleLabel}, {levelLabel}
          </span>
        </div>
        <div className="flex justify-between border-b border-gray-100 pb-2">
        ...
        <div className="flex justify-between">
          ...
          <span className="text-[11px] text-content/70">
            {ctx.selectedNewUserStores.length} selected
          </span>
        </div>
      </div>
