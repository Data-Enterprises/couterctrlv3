...
import { setAvailableEmailDetails, setAvailableUsernameDetails, setUserInfo } from "../../../../../features/usersSlice";
...
const StepUserInfo = ({ onContinue }: StepUserInfoProps) => {
      ...
      .then((resp) => {
        ...
        ctx.dispatch(
          setAvailableUsernameDetails(
            j.error === 0
              ? { availableUsernameText: "- Available", usernameTextColor: "text-emerald-600" }
              : { availableUsernameText: "- Not available", usernameTextColor: "text-red-600" },
          ),
        );
      })
      ...
      .then((resp) => {
        ...
        ctx.dispatch(
          setAvailableEmailDetails(
            j.error === 0
              ? { availableEmailText: "- Available", emailTextColor: "text-emerald-600" }
              : { availableEmailText: "- Not available", emailTextColor: "text-red-600" },
          ),
        );
      })
      .catch((err: JsonError) => toast.error(err.message));
  ...
  return (
    <div className="max-w-[440px]">
      <div className="mb-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          {roles.map((r) => (
            <button
              key={r.value}
              onClick={() => ctx.dispatch(setUserInfo({ key: "role", value: Number(r.value) }))}
              className={`text-[11px] px-3 py-1 rounded-full ${ctx.userInfo.role === Number(r.value) ? "bg-[#1e2a4a] text-custom-white" : "bg-white border border-gray-200 text-content"}`}
            >
              {r.label}
            </button>
          ))}
      ...
      <div className="mb-4">
        <div className="text-[11px] font-medium text-content mb-1">User level</div>
        <div className="flex flex-wrap gap-1.5">
          {availableLevels.map((l) => (
            <button
              key={l.id}
              onClick={() => ctx.dispatch(setUserInfo({ key: "user_level", value: l.id }))}
              className={`text-[11px] px-3 py-1 rounded-full ${ctx.userInfo.user_level === l.id ? "bg-[#1e2a4a] text-custom-white" : "bg-white border border-gray-200 text-content"}`}
            >
              {l.name}
            </button>
          ))}
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        <Input
          ...
          setValue={(v) => ctx.dispatch(setUserInfo({ key: "username", value: v }))}
        ...
        <Input
          ...
          setValue={(v) => ctx.dispatch(setUserInfo({ key: "email", value: v }))}
          className="py-1.5 text-[13px]"
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        <Input
          ...
          setValue={(v) => ctx.dispatch(setUserInfo({ key: "first_name", value: v }))}
          className="py-1.5 text-[13px]"
        />
        <Input
          label="Last name"
          value={ctx.userInfo.last_name}
          setValue={(v) => ctx.dispatch(setUserInfo({ key: "last_name", value: v }))}
          className="py-1.5 text-[13px]"
        />
      </div>
      <div className="grid grid-cols-2 gap-3 mb-5">
        <PasswordInput
          ...
          setText={(v) => ctx.dispatch(setUserInfo({ key: "password", value: v }))}
        ...
        <PasswordInput
          ...
          setText={(v) => ctx.dispatch(setUserInfo({ key: "confirm_password", value: v }))}
          leftCompare={ctx.userInfo.password}
          rightCompare={ctx.userInfo.confirm_password}
          className="py-1.5"
        />
