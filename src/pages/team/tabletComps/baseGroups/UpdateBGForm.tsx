...
const UpdateBGForm = () => {
  ...
  return (
    <div data-testid="update-bg-form-container">
      <div className="bg-custom-white p-2 rounded-lg shadow-lg w-[75%]">
        {/* Companies */}
        ...
        {baseGroups.length ? (
          <div className="text-[13px] my-4">
            ...
            <div
              className={`select-none rounded-lg p-1 max-h-[60vh] grid grid-cols-2 overflow-y-auto`}
            >
              {baseGroups.map((bg, i) => (
                <div
                  key={bg.id}
                  data-testid={`bg-option-${i}`}
                  className={`${selectedBgID === bg.id && "bg-[rgb(30,45,80)]/75 text-custom-white"} rounded-full py-1 pl-2 border-b transition-all duration-200 cursor-pointer`}
        ...
        {showForm && (
          <div className="space-y-2">
            ...
            <button
              ...
              className={`btn-themeBlue bg-[rgb(30,45,80)] border-[rgb(30,45,80)] w-full ${!selectedBgID && "opacity-50 pointer-events-none"}`}
