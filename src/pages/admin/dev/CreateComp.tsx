...
const CreateComp = () => {
  ...
  const alreadyExists = context.companies.some(
    (c) =>
      c.name.toLowerCase() === context.companyForm.name.toLowerCase() &&
      context.companyForm.name !== "",
  );

  const { address, city, contact_email, state, zip, phone, name } =
    context.companyForm;
  ...
  return (
    <div className="flex flex-1 min-h-0">
      <CompanyPicker
        ...
        mode="reference"
        collisionName={context.companyForm.name}
      />
