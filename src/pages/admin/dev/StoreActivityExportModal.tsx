...
const buildStoreActivityCsv = (stores: StoreWithActivity[], label: string) => {
  const headers = ["Store ID", "Store Name", "Total Days", "Active Days", "Days Missing"];
  const rows = stores.map((s) => [
    s.storeid, s.store_name, s.total_days_in_range, s.active_days, s.inactive_or_missing_days,
  ]);
...
const StoreActivityExportModal = ({ onClose, companyName, stores }: StoreActivityExportModalProps) => {
  ...
  const handleDownload = () => {
    ...
    if (selected.has("all")) sections.push(buildStoreActivityCsv(stores, `All Stores — ${companyName}`));
    if (selected.has("missing")) sections.push(buildStoreActivityCsv(missingStores, `Missing Only — ${companyName}`));
      ...
      >
        ...
        <div className="px-4 pt-4 pb-2 space-y-3">
          ...
          <label className="flex items-start gap-3 cursor-pointer group">
            ...
            />
            <div>
              <p className="text-[13px] font-medium text-content group-hover:text-[#1e2a4a] transition-colors">All stores</p>
              <p className="text-[12px] text-content mt-0.5">{stores.length} stores for {companyName}</p>
            </div>
          </label>

          <label className={`flex items-start gap-3 ${missingStores.length ? "cursor-pointer" : "opacity-40 cursor-not-allowed"} group`}>
            <input
              type="checkbox"
              checked={selected.has("missing")}
              disabled={!missingStores.length}
              onChange={() => setSelected((p) => { const n = new Set(p); n.has("missing") ? n.delete("missing") : n.add("missing"); return n; })}
              className="mt-0.5 h-3.5 w-3.5 rounded border-gray-300 accent-[#1e2a4a] cursor-pointer flex-shrink-0"
            />
            <div>
              <p className="text-[13px] font-medium text-content group-hover:text-[#1e2a4a] transition-colors">Missing only</p>
              <p className="text-[12px] text-content mt-0.5">
                {missingStores.length ? `${missingStores.length} stores with 1+ missing days` : "No stores currently missing days"}
              </p>
            </div>
          </label>
        </div>
