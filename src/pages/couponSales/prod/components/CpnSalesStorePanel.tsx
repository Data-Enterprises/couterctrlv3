...
const CpnSalesStorePanel = ({ rows, totals, rangeLabel, onOpenSearch }: Props) => {
  ...
  const rawThreshold = useAppSelector((s) => s.prod.couponSales.threshold);
  const activeThreshold = rawThreshold ?? COUPON_THRESHOLD_DEFAULT;
  const rawTrendThreshold = useAppSelector((s) => s.prod.couponSales.trendThreshold);
  const metric = useAppSelector((s) => s.prod.couponSales.metric);
  ...
  const tierFilter = useAppSelector((s) => s.prod.couponSales.tierFilter);
  const storeFilter = useAppSelector((s) => s.prod.couponSales.storeFilter);
  const selectedStoreKey = useAppSelector((s) => s.prod.couponSales.selectedStoreKey);
