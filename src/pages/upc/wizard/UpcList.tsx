...
const UpcList = () => {
  ...
  const [styling, setStyling] = useState<string>("h-[280px] w-[450px]");

  const upc = useAppSelector((state) => state.upc);

  // To set the height and width of the wizard based on the step
  useEffect(() => {
    if (upc.index === 0) setStyling("h-[280px] w-[450px]");
    if (upc.index === 1) setStyling("h-[500px] w-[550px]");
        ...
        <>
          ...
          >
            <StepOne
              ...
              setFile={setFile}
            />
            <StepTwo className={"h-[500px] w-[550px]"} getData={getData} />
            <StepThree />
          </UpcWizard>
        </>
      )}
