...
import {
  ...
  setLpSevFilter,
  toggleLpType,
} from "../../../features/dev/devLpActionsSlice";
import type { LpSevFilter } from "../../../features/dev/devLpActionsSlice";
...
const StoreRow = ({ row }: { row: ExceptionRow }) => {
  ...
  const selectedId = useAppSelector((s) => s.dev.lpActions.selectedId);
  return (
    <button
...
const TypeSection = ({ group }: { group: TypeGroup }) => {
  const dispatch = useAppDispatch();
  const expandedTypes = useAppSelector((s) => s.dev.lpActions.expandedTypes);
