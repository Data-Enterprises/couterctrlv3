...
const TransactionsGrid = () => {
  ...
  return (
    <div
      ...
      className="bg-custom-white p-3 rounded-lg shadow-lg h-[90%] relative"
    >
      <ExportModal
        isOpen={modalOpen}
      ...
      <div className="max-h-[calc(100%-3rem)] overflow-y-auto">
        ...
        <div className="divide-y divide-content/10">
          {filteredOverviews.map((fo, i) => (
            <div
              key={i}
              className="grid grid-cols-[1fr_repeat(4,1fr)_repeat(3,0.8fr)] px-3 py-3 md:py-4 text-[13px] last:rounded-b-lg"
              onClick={() => handleTransIDClick(fo)}
            >
              <div className="font-medium underline cursor-pointer truncate pr-2 hover:text-blue-600">
                {fo.transaction_id}
              </div>
              <div className="truncate">{formatDate(fo.sale_date)}</div>
              <div className="truncate">{fo.sale_type}</div>
              <div className="truncate">{fo.store_number}</div>
      ...
      <div className="absolute bottom-2 left-3 right-3 flex gap-2 md:gap-3 pointer-events-none md:pointer-events-auto">
        <button
          ...
          className="btn-themeGreen py-1.5 px-3 flex-1 md:flex-none text-sm md:text-base pointer-events-auto"
