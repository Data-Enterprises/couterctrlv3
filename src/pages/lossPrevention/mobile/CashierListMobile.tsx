...
const CashierListMobile = ({ onBack, onSelectCashier }: Props) => {
  ...
  const noSale =
    lp.selectedSaleType.toLowerCase().replace(/[^a-z]/g, "") === "nosale";
  ...
  const isSelectedStore = (r: { storeid: number; store_number: string }) =>
    r.storeid === lp.selectedStoreId &&
    r.store_number === lp.selectedStoreNumber;
  ...
  const storeName = applyStoreNumberToName(
    assignedStores.find((s) => s.storeid === lp.selectedStoreId)?.store_name ??
      String(lp.selectedStoreId),
    lp.selectedStoreNumber,
    [
      ...new Set(
        lp.cashierDetails
          .filter((d) => d.storeid === lp.selectedStoreId)
          .map((d) => d.store_number),
      ),
    ],
  );
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div
        className="flex-shrink-0 grid divide-x divide-gray-100 bg-custom-white border-b border-gray-100"
        style={{
          gridTemplateColumns: stdCols ? "repeat(4, 1fr)" : "repeat(2, 1fr)",
        }}
      >
        <KpiCell
          label="Trans"
          value={detail ? detail.transaction_count.toLocaleString() : "—"}
          pct={
            detail && trend
              ? trendPct(detail.transaction_count, trend.transaction_count)
              : undefined
          }
          baseline={
            trend ? trend.transaction_count.toLocaleString() : undefined
          }
        />
        <KpiCell
          label="Qty"
          value={detail ? detail.total_items.toLocaleString() : "—"}
          pct={
            detail && trend
              ? trendPct(detail.total_items, trend.total_items)
              : undefined
          }
          baseline={trend ? trend.total_items.toLocaleString() : undefined}
        />
        {stdCols && (
          <KpiCell
            label="Total"
            value={detail ? formatCurrency2(detail.amount) : "—"}
            pct={
              detail && trend
                ? trendPct(detail.amount, trend.amount, true)
                : undefined
            }
            baseline={
              trend ? formatCurrency2(Math.abs(trend.amount)) : undefined
            }
          />
        )}
        {stdCols && (
          <KpiCell
            label="Avg ticket"
            value={detail ? formatCurrency2(detail.average_dollars) : "—"}
            pct={
              detail && trend
                ? trendPct(detail.average_dollars, trend.average_dollars, true)
                : undefined
            }
            baseline={
              trend
                ? formatCurrency2(Math.abs(trend.average_dollars))
                : undefined
            }
            last
          />
      ...
      {/* {peerAvgs && (
        ...
        <div className="flex-shrink-0 px-3 py-[9px] bg-gray-100 border-b border-gray-200 flex items-center justify-between">
          <span className="text-[10px] font-semibold uppercase tracking-wide text-content/85">Cashier Averages</span>
          <div className="flex items-center gap-1.5">
            <div className="flex items-center gap-1"><div className="w-[6px] h-[6px] rounded-[2px] bg-red-400 flex-shrink-0" /><span className="text-[10px] font-semibold uppercase tracking-wide text-content/85">{noSale ? "0" : "0–1"}</span></div>
            <div className="flex items-center gap-1"><div className="w-[6px] h-[6px] rounded-[2px] bg-amber-400 flex-shrink-0" /><span className="text-[10px] font-semibold uppercase tracking-wide text-content/85">1</span></div>
            <div className="flex items-center gap-1"><div className="w-[6px] h-[6px] rounded-[2px] bg-emerald-400 flex-shrink-0" /><span className="text-[10px] font-semibold uppercase tracking-wide text-content/85">{noSale ? "2" : "3–4"}</span></div>
            <span className="text-[10px] font-semibold uppercase tracking-wide text-content/85">below/at store avg</span>
          </div>
        </div>
          ...
          visible.map((g) => {
            const metrics = noSale
              ? [
                  {
                    label: "Trans",
                    value: g.trans.value.toLocaleString(),
                    isPass: g.trans.isPass,
                  },
                  {
                    label: "Qty",
                    value: g.qty.value.toLocaleString(),
                    isPass: g.qty.isPass,
                  },
                ]
              : [
                  ...
                  {
                    label: "Total",
                    value: formatCurrency2(Math.abs(g.sales.value)),
                    isPass: g.sales.isPass,
                  },
                  {
                    label: "Avg $",
                    value: formatCurrency2(Math.abs(g.avgTicket.value)),
                    isPass: g.avgTicket.isPass,
                  },
                ];
              ...
              >
                <div className="flex items-center gap-2.5 mb-2">
                  ...
                  <div className="text-[13px] font-medium text-content truncate">
                    {g.cashier_name}{" "}
                    <span className="text-content/85 font-normal text-[11px]">
                      #{g.cashier_number}
                    </span>
                  </div>
                </div>
