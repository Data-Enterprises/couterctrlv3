...
const TransactionsMobile = ({ onBack }: Props) => {
  ...
  const cashierGrade =
    grades.find(
      (g) => g.cashier_number === lp.selectedCashier.cashier_number,
    ) ?? null;
  ...
  useEffect(() => {
    ...
    setFilteredOverviews(
      lp.transOverviews.filter((item) => {
        const matchCashier = selectedCashierNum
          ? item.cashier_number === selectedCashierNum
          : true;
        ...
        const matchQty = qty
          ...
          : true;
        return (
          matchCashier && matchDate && matchTransId && matchSales && matchQty
        );
      }),
    );
  }, [
  ...
  const visible = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      let av = 0,
        bv = 0;
      if (sortCol === "date") {
        av = new Date(a.sale_date).getTime();
        bv = new Date(b.sale_date).getTime();
      }
      if (sortCol === "qty") {
        av = a.qty ?? 0;
        bv = b.qty ?? 0;
    ...
    getCashierTransaction(
      ...
      .then((resp) => {
        ...
        if (j.error === 0 && j.transaction.length > 0) {
          ...
          dispatch(
            actions.setTransactionDrillDown([reduceTransactions(transactions)]),
          );
        }
      })
      .catch((err: JsonError) => {
  ...
  const cashierName =
    lp.cashiers.find(
      (c) => c.cashier_number === lp.selectedCashier.cashier_number,
    )?.cashier_name ?? "";
  const noSale =
      ...
      <div className="bg-[#1e2a4a] px-4 pt-3 pb-4 flex items-start gap-3 flex-shrink-0">
        ...
        <div className="min-w-0 flex-1">
          <div className="text-custom-white font-semibold text-[15px] truncate">
            {cashierName}{" "}
            <span className="text-custom-white/85 font-normal text-[12px]">
              #{lp.selectedCashier.cashier_number}
            </span>{" "}
            <span className="text-custom-white/85 font-normal text-[11px]">
      ...
      {cashierGrade && (
        <>
          ...
          <div
            ...
            style={{
              gridTemplateColumns: noSale ? "repeat(2, 1fr)" : "repeat(4, 1fr)",
            }}
          >
            {[
              {
                label: "Trans",
                value: cashierGrade.trans.value.toLocaleString(),
                pct: cashierGrade.trans.pct,
                ...
                ? [
                    {
                      ...
                      value: formatCurrency2(
                        Math.abs(cashierGrade.sales.value),
                      ),
                      pct: cashierGrade.sales.pct,
                      avg: formatCurrency2(Math.abs(cashierGrade.sales.avg)),
                    },
                    {
                      label: "Avg ticket",
                      value: formatCurrency2(
                        Math.abs(cashierGrade.avgTicket.value),
                      ),
                      pct: cashierGrade.avgTicket.pct,
                      avg: formatCurrency2(
                        Math.abs(cashierGrade.avgTicket.avg),
                      ),
                    },
                  ]
                : []),
